...
ppmData image = concat (map encodeRGB (concat image))

writePPM :: FilePath -> Image -> IO ()
writePPM path image@(x:_) = do
                    imageHandle <- openBinaryFile path WriteMode
                    case (validImage image) of 
                        Just (w,h) -> hPutStr imageHandle (ppmHeader (w,h))
                        Nothing -> do 
                                        hClose imageHandle
                                        error "Image not Valid"
                    hPutStr imageHandle (ppmData image)
                    hClose imageHandle
